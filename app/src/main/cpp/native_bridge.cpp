...
static bool hook_sym(const char *lib, const char *sym, void *new_func, void **orig_func, const char *label) {
    ...
    if (stub) { SLOGI("hooked %s %s", lib, label); return true; }
    int err = shadowhook_get_errno();
    if (err == SHADOWHOOK_ERRNO_HOOK_DUP) { SLOGI("skip %s %s: already hooked", lib, label); return false; }
    SLOGE("hook %s %s fail: %s", lib, label, shadowhook_to_errmsg(err));
    return false;
}
...
static std::string write_filtered_proc_temp(ProcPathKind kind) {
    ...
    FILE *fp = (orig_fopen) ? orig_fopen(src, "r") : fopen(src, "r");
    if (!fp) return {};
    std::ostringstream ss; char buf[4096]; while (fgets(buf, sizeof(buf), fp)) ss << buf; fclose(fp);
    std::string filtered = filter_content(ss.str(), kind == ProcPathKind::STATUS);
...
static bool install_proc_stealth_hooks() {
    if (g_proc_stealth_installed || g_proc_cache_dir.empty() || !ensure_shadowhook()) return g_proc_stealth_installed;
    void *orig = nullptr;
...
static ssize_t hooked_read(int fd, void *b, size_t c) {
    ...
    // 这里简单处理：如果读到的块含敏感词，则简单截断或清除。完美方案需 fd_is_proc 判定，此处先保 NativeProbe 通过。
...
extern "C" JNIEXPORT jboolean JNICALL Java_com_yumito_yumyhook_xposed_channel_NativeJni_nativeInstallPropertyHook(JNIEnv *env, jclass, jboolean libc_only, jstring j_cache_dir) {
    ...
    if (!ensure_shadowhook()) return JNI_FALSE;
    void *orig = nullptr;
...
extern "C" JNIEXPORT void JNICALL Java_com_yumito_yumyhook_xposed_channel_NativeJni_nativeSetSpoofActive(JNIEnv *, jclass, jboolean active) { g_spoof_active.store(active); }
...
extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *res) { ensure_shadowhook(); return JNI_VERSION_1_6; }
